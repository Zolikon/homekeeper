import { getFlight, getVacationInfo } from "@/app/__backend/VacationService";
...
export default async function FlightOutPage() {
  const [flight, info] = await Promise.all([getFlight(VACATION_FLIGHT_OUT_ID), getVacationInfo()]);
  if (!info) return null;
  return (
    <div>
      ...
      <FlightView flight={flight} vacationDate={info.startDate} />
    </div>
  );
}
